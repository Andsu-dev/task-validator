...
class TaskValidatorAgent {
    ...
    async validateTask(context) {
        try {
            logger_1.logger.info(`Starting validation for task ${context.rules.taskId}`);
            const prompt = this.buildValidationPrompt(context);
            // Log do prompt enviado para a IA
            logger_1.logger.info(`Prompt sent to AI for task ${context.rules.taskId}`, {
            ...
            logger_1.logger.info(`AI response received for task ${context.rules.taskId}`, {
                responseLength: response.content?.length || 0,
                analysisCount: agentResponse.analysis.length,
                overallCompleteness: agentResponse.overallCompleteness
            });
            logger_1.logger.info(`Validation completed for task ${context.rules.taskId}`, {
            ...
            logger_1.logger.error('Error during task validation', error);
            throw error;
        }
    }
...
//# sourceMappingURL=TaskValidatorAgent.js.map